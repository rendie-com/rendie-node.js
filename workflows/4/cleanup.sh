...
# 🌟 关键修复：彻底解除容器挂载导致的 Git 安全目录所有权报错
git config --global --add safe.directory "*"

echo "🧹 [开始执行自动化收尾与空间回容]..."

# 1. 动态剥离过期的【1688采集箱】缓存层，只留最新的单帧基准
OLD_1688_CACHES=$(gh cache list --limit 100 --json id,key --jq '[.[] | select(.key | startswith("1688-crawler-"))] | .[1:] | .[].id' 2>/dev/null || true)
if [ -n "$OLD_1688_CACHES" ]; then
  echo "🗑️ 发现历史【1688采集箱】残留缓存，正在擦除..."
  echo "$OLD_1688_CACHES" | xargs -I {} gh cache delete {}
  echo "✅ 历史缓存清理完毕。"
else
  echo "ℹ️ 【1688采集箱】历史空间完美纯净。"
fi

# 2. 回收 50 次以前的历史流水日志堆积
